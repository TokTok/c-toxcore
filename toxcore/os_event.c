...
static bool os_ev_add(void *self, Socket sock, Ev_Events events, void *data)
{
    ...
    OS_Ev_Registration *reg = &os_ev->regs[os_ev->regs_count];
    reg->sock = sock;
    reg->events = events;
    reg->data = data;
...
static bool os_ev_del(void *self, Socket sock)
{
    ...
    for (uint32_t i = 0; i < os_ev->regs_count; ++i) {
        if (net_socket_to_native(os_ev->regs[i].sock) == net_socket_to_native(sock)) {
            ...
            if (i < os_ev->regs_count - 1) {
                struct epoll_event ep_ev;
                ep_ev.events = events_to_epoll(os_ev->regs[i].events);
                ep_ev.data.ptr = &os_ev->regs[i];

                if (epoll_ctl(os_ev->epoll_fd, EPOLL_CTL_MOD, net_socket_to_native(os_ev->regs[i].sock), &ep_ev)
                        == -1) {
                    /* TODO: what should we do here? logging maybe? */
                }
            }
...
static bool os_ev_add(void *self, Socket sock, Ev_Events events, void *data)
{
    ...
    if (os_ev->regs_count == os_ev->regs_capacity) {
        ...
        // We use h_events to store the AFD_Poll_Handle_Info array.
        // h_events is defined as WSAEVENT* in the struct, but we cast it to store AFD handles.
    ...
    os_ev->regs[os_ev->regs_count].sock = sock;
    os_ev->regs[os_ev->regs_count].events = events;
    os_ev->regs[os_ev->regs_count].data = data;
...
static int32_t os_ev_run(void *self, Ev_Result *results, uint32_t max_results, int32_t timeout_ms)
{
    ...
    size_t info_size = sizeof(AFD_Poll_Info) + (os_ev->regs_count - 1) * sizeof(AFD_Poll_Handle_Info);
