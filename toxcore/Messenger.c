                         ...
                         Messenger_Options *options, Messenger_Error *error)
{
    ...
    const Crypto_Handshake_Mode handshake_mode = options->noise_compatibility_enabled
        ? CRYPTO_HANDSHAKE_MODE_NOISE_BOTH
        : CRYPTO_HANDSHAKE_MODE_NOISE_ONLY;
    Net_Crypto *net_crypto = new_net_crypto(m->log, m->mem, m->rng, m->ns, m->mono_time, m->net, m->dht, &m_dht_funcs, &options->proxy_info, m->tcp_np, handshake_mode);
