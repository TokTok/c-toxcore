...
static bool operator!=(PublicKey const &pk1, PublicKey const &pk2)
{
...
static bool operator==(PublicKey::Base const &pk1, PublicKey const &pk2)
{
    return pk1 == pk2.base();
}

static std::ostream &operator<<(std::ostream &out, PublicKey const &pk)
{
    out << '"';
    for (uint8_t byte : pk) {
...
static bool operator==(Node_format const &a, Node_format const &b)
{
    return std::memcmp(&a, &b, sizeof(Node_format)) == 0;
}

static std::ostream &operator<<(std::ostream &out, IP const &v)
{
    Ip_Ntoa ip_str;
...
TEST(AddToList, KeepsKeysInOrder)
{
    ...
    auto const by_distance = [&cmp_pk](auto const &node1, auto const &node2) {
        return id_closest(cmp_pk.data(), node1.public_key, node2.public_key) < 2;
    };
