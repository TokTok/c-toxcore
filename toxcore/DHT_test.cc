...
TEST(AddToList, OverridesKeysWithCloserKeys) {
  ...
  EXPECT_TRUE(add_to_list(nodes.data(), nodes.size(), keys[0].data(), IP_Port().data(), self_pk.data()));
  EXPECT_TRUE(add_to_list(nodes.data(), nodes.size(), keys[1].data(), IP_Port().data(), self_pk.data()));
  EXPECT_TRUE(add_to_list(nodes.data(), nodes.size(), keys[2].data(), IP_Port().data(), self_pk.data()));
  EXPECT_TRUE(add_to_list(nodes.data(), nodes.size(), keys[3].data(), IP_Port().data(), self_pk.data()));
  ...
  EXPECT_FALSE(add_to_list(nodes.data(), nodes.size(), keys[4].data(), IP_Port().data(), self_pk.data()));
  // 5 is closer than all except key 0
  EXPECT_TRUE(add_to_list(nodes.data(), nodes.size(), keys[5].data(), IP_Port().data(), self_pk.data()));
