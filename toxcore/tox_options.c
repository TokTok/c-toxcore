...
static bool options_set_string(bool experimental_owned_data, const char **options_str, char **options_owned_str, const char *user_str)
{
...
{
    return options_set_string(options->experimental_owned_data, &options->proxy_host, &options->owned_proxy_host, proxy_host);
}
...
static bool options_set_array(bool experimental_owned_data, const uint8_t **options_data, size_t *options_length, uint8_t **options_owned_data, const uint8_t *user_data, size_t user_length)
{
    if (!experimental_owned_data) {
...
{
    return options_set_array(options->experimental_owned_data, &options->proxy_socks5_username, &options->proxy_socks5_username_length, &options->owned_proxy_socks5_username, username, length);
}
...
{
    return options_set_array(options->experimental_owned_data, &options->proxy_socks5_password, &options->proxy_socks5_password_length, &options->owned_proxy_socks5_password, password, length);
}
...
{
    return options_set_array(options->experimental_owned_data, &options->savedata_data, &options->savedata_length, &options->owned_savedata_data, savedata_data, length);
}
