...
const Network system_network = {&system_network_funcs};
...
static int net_sendto(
        const Network *ns, const Logger *log,
        Socket sock, const uint8_t *buf, size_t len, const Network_Addr *addr, const IP_Port *ip_port)
{
    const int res = ns->funcs->sendto(ns->obj, sock.sock, buf, len, addr);
    loglogdata(log, "O=>", buf, len, ip_port, res);
    return res;
}
...
int send_packet(const Networking_Core *net, const IP_Port *ip_port, Packet packet)
{
    ...
    const long res = net_sendto(net->ns, net->log, net->sock, packet.data, packet.length, &addr, &ipp_copy);
    assert(res <= INT_MAX);
    return (int)res;
}
...
{
    int count = ns->funcs->recvbuf(ns->obj, sock.sock);
