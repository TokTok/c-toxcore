...
static int inet_pton4(const char *addrString, struct in_addr *addrbuf)
{
    ...
    char addrStringMut[128] = {0};
    memcpy(addrStringMut, addrString, min_u16(sizeof(addrStringMut) - 1, strlen(addrString)));

    if (WSAStringToAddress(addrStringMut, AF_INET, nullptr, (LPSOCKADDR)&saddr, &len)) {
        return 0;
    }
...
static int inet_pton6(const char *addrString, struct in6_addr *addrbuf)
{
    ...
    if (WSAStringToAddress(addrStringMut, AF_INET6, nullptr, (LPSOCKADDR)&saddr, &len)) {
        return 0;
    }
