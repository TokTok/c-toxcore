...
non_null()
static int sys_getaddrinfo(void *obj, const char *address, int family, Network_Addr **addrs)
{
    ...
    *addrs = nullptr;
    ...
    // hints.ai_protocol = SOCK_DGRAM; // type of socket Tox uses.
...
non_null()
static int sys_getaddrinfo_dummy(void *obj, const char *address, int family, Network_Addr **addrs)
{
    return 0;
}
...
int32_t net_getipport(const Network *ns, const Memory *mem, const char *node, IP_Port **res, int tox_type)
{
    ...
    *res = nullptr;
    ...
    Network_Addr* addrs = nullptr;
    const int rc = ns->funcs->getaddrinfo(ns->obj, node, AF_UNSPEC, &addrs);
    ...
    for (int i = 0; i < rc && count < max_count; i++) {
        ...
        // TODO(Green-Sky): add socktype/protocol to getaddrinfo
