...
#endif /* GNU */
char *owner net_new_strerror(const Memory *mem, int error)
{
    char tmp[256];
    ...
    char *owner str = (char *owner)mem_balloc(mem, retstr_len + 1);
...
void net_kill_strerror(const Memory *mem, char *owner strerror)
{
#ifdef OS_WIN32
    LocalFree(strerror);
#else
