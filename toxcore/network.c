...
// TODO(https://github.com/TokTok/c-toxcore/pull/2595): Remove casts.
static const Family family_unspec = (force Family)NET_FAMILY_UNSPEC;
static const Family family_ipv4 = (force Family)NET_FAMILY_IPV4;
static const Family family_ipv6 = (force Family)NET_FAMILY_IPV6;
static const Family family_tcp_server = (force Family)NET_FAMILY_TCP_SERVER;
static const Family family_tcp_client = (force Family)NET_FAMILY_TCP_CLIENT;
static const Family family_tcp_ipv4 = (force Family)NET_FAMILY_TCP_IPV4;
static const Family family_tcp_ipv6 = (force Family)NET_FAMILY_TCP_IPV6;
static const Family family_tox_tcp_ipv4 = (force Family)NET_FAMILY_TOX_TCP_IPV4;
static const Family family_tox_tcp_ipv6 = (force Family)NET_FAMILY_TOX_TCP_IPV6;
...
Family family_from_net_family(Net_Family family)
{
    return (force Family)family;
}

uint8_t family_to_int(Family family)
{
    // TODO(https://github.com/TokTok/c-toxcore/pull/2595): Remove force.
    return (force uint8_t)family;
}
...
{
    return family == family_tox_tcp_ipv4;
}

bool net_family_is_tox_tcp_ipv6(Family family)
{
    return family == family_tox_tcp_ipv6;
}
...
int send_packet(const Networking_Core *net, const IP_Port *ip_port, Packet packet)
{
    ...
    if (net_family_is_ipv4(net->family) && !net_family_is_ipv4(ipp_copy.ip.family)) {
        ...
        LOGGER_WARNING(net->log, "attempted to send message with network family %d (probably IPv6) on IPv4 socket (%s)",
                       ipp_copy.ip.family, net_ip_ntoa(&ipp_copy.ip, &ip_str));
        return -1;
    }
    ...
    if (net_family_is_ipv4(ipp_copy.ip.family)) {
        ...
        LOGGER_ERROR(net->log, "unknown address type: %d", ipp_copy.ip.family);
        return -1;
    }
...
const char *net_ip_ntoa(const IP *ip, Ip_Ntoa *ip_str)
{
    ...
    if (!ip_parse_addr(ip, ip_str->buf, sizeof(ip_str->buf))) {
        snprintf(ip_str->buf, sizeof(ip_str->buf), "(IP invalid, family %u)", ip->family);
        ip_str->length = (uint16_t)strlen(ip_str->buf);
        return ip_str->buf;
    }
