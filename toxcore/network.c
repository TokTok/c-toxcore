...
{
#if defined(OS_WIN32)
    return closesocket(net_socket_to_system(sock));
#else  // !OS_WIN32
    return close(net_socket_to_system(sock));
#endif /* OS_WIN32 */
}
...
{
    return net_socket_from_system(accept(net_socket_to_system(sock), nullptr, nullptr));
}

non_null()
static int sys_bind(void *obj, Socket sock, const Network_Addr *addr)
{
    return bind(net_socket_to_system(sock), (const struct sockaddr *)&addr->addr, addr->size);
}

non_null()
static int sys_listen(void *obj, Socket sock, int backlog)
{
    return listen(net_socket_to_system(sock), backlog);
}
...
#ifdef OS_WIN32
    u_long count = 0;
    ioctlsocket(net_socket_to_system(sock), FIONREAD, &count);
#else
    int count = 0;
    ioctl(net_socket_to_system(sock), FIONREAD, &count);
#endif /* OS_WIN32 */
...
{
    return recv(net_socket_to_system(sock), (char *)buf, len, MSG_NOSIGNAL);
}

non_null()
static int sys_send(void *obj, Socket sock, const uint8_t *buf, size_t len)
{
    return send(net_socket_to_system(sock), (const char *)buf, len, MSG_NOSIGNAL);
}

non_null()
static int sys_sendto(void *obj, Socket sock, const uint8_t *buf, size_t len, const Network_Addr *addr)
{
    return sendto(net_socket_to_system(sock), (const char *)buf, len, 0, (const struct sockaddr *)&addr->addr, addr->size);
}
...
static int sys_recvfrom(void *obj, Socket sock, uint8_t *buf, size_t len, Network_Addr *addr)
{
    ...
    const int ret = recvfrom(net_socket_to_system(sock), (char *)buf, len, 0, (struct sockaddr *)&addr->addr, &size);
    addr->size = size;
    return ret;
}
...
{
    return net_socket_from_system(socket(domain, type, proto));
}
...
static int sys_socket_nonblock(void *obj, Socket sock, bool nonblock)
{
    ...
    return ioctlsocket(net_socket_to_system(sock), FIONBIO, &mode);
#else
    return fcntl(net_socket_to_system(sock), F_SETFL, O_NONBLOCK, nonblock ? 1 : 0);
#endif /* OS_WIN32 */
}
...
static int sys_getsockopt(void *obj, Socket sock, int level, int optname, void *optval, size_t *optlen)
{
    ...
    const int ret = getsockopt(net_socket_to_system(sock), level, optname, (char *)optval, &len);
    *optlen = len;
    return ret;
}
...
{
    return setsockopt(net_socket_to_system(sock), level, optname, (const char *)optval, optlen);
}
