...
static void loglogdata(const Logger *log, const char *message, const uint8_t *buffer,
                       uint16_t buflen, IP_Port ip_port, ssize_t res)
{
...
#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
    const ssize_t res = fuzz_sendto(net->sock.socket, (const char *)packet.data, packet.length, 0,
                                    (struct sockaddr *)&addr, addrsize);
#else
    const ssize_t res = sendto(net->sock.socket, (const char *)packet.data, packet.length, 0,
                               (struct sockaddr *)&addr, addrsize);
#endif
