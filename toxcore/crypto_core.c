...
int32_t encrypt_precompute(const uint8_t *public_key, const uint8_t *secret_key,
                           uint8_t *shared_key)
{
    uint8_t temp_shared_key[CRYPTO_SHARED_KEY_SIZE];
    int32_t res = crypto_box_beforenm(temp_shared_key, public_key, secret_key);
    memcpy(shared_key, temp_shared_key, CRYPTO_SHARED_KEY_SIZE);
    return res;
}
