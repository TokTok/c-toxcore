 ...
 */
Net_Crypto *new_net_crypto(const Logger *log, const Random *rng, Mono_Time *mono_time, DHT *dht, const TCP_Proxy_Info *proxy_info)
{
    if (dht == nullptr) {
        return nullptr;
    }
    ...
    temp->tcp_c = new_tcp_connections(log, rng, mono_time, dht_get_self_secret_key(dht), proxy_info);
