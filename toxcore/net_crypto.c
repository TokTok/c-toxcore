...
// TODO(irungentoo): TCP, etc...
    ...
    if (!net_family_is_unspec(ip_port.ip.family)) {
        ...
        if (direct_connected) {
            ...
            pthread_mutex_unlock(conn->mutex);
            LOGGER_ERROR(c->log, "sending packet of length %d failed", length);
            return -1;
        }
                                    ...
                                    uint8_t congestion_control)
{
    ...
    if (conn->maximum_speed_reached && congestion_control) {
        LOGGER_ERROR(c->log, "congestion control: maximum speed reached on crypt connection %d", crypt_connection_id);
        return -1;
    }
