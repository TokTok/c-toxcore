        ...
        const uint8_t *data, uint16_t length, uint8_t packet_type, uint64_t message_id)
{
    ...
    if (length == 0) {
        if (data != nullptr) {
            LOGGER_ERROR(log, "Got non-null data with zero length (type %d)", packet_type); // should never happen
            return false;
        }
        ...
        if (data == nullptr) {
            LOGGER_ERROR(log, "Got null data with non-zero length (type %u)", packet_type); // should never happen
            return false;
        }
                                ...
                                uint16_t length, uint8_t packet_type, uint64_t message_id)
{
    ...
    if (!create_array_entry(log, mono_time, ary_entry, data, length, packet_type, message_id)) {
        return false;
    }
