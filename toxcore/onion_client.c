                                    ...
                                    void *userdata)
{
    ...
    if (num > onion_c->num_friends) {
        LOGGER_WARNING(onion_c->logger, "invalid announce response: sendback number wrong or sendback check failed: %lu > %u",
                       (unsigned long)num, (unsigned int)onion_c->num_friends);
        return 1;
    }
    ...
    if (num == 0) {
        ...
        if (!onion_c->friends_list[num - 1].is_valid) {
            LOGGER_WARNING(onion_c->logger, "friend number %lu is invalid", (unsigned long)(num - 1));
            return 1;
        }
...
non_null()
static bool node_list_contains(const Node_format *const *nodes, uint16_t nodes_size, const uint8_t *public_key)
{
    for (uint16_t i = 0; i < nodes_size; ++i) {
        if (pk_equal(nodes[i]->public_key, public_key)) {
            return true;
        }
    }
...
static void do_announce(Onion_Client *onion_c)
{
    ...
    if (count <= MAX_ONION_CLIENTS_ANNOUNCE / 2
        ...
        const Node_format *targets[MAX_ONION_CLIENTS_ANNOUNCE / 2];
