...
#define MAX_RUN_COUNT_EXPONENT 12.0
...
static void do_friend(Onion_Client *onion_c, uint16_t friendnum)
{
    ...
    if (!friend_is_new) {
        ...
        interval = min_u32((uint32_t)c, ANNOUNCE_FRIEND_MAX_INTERVAL);
    } else {
...
static void do_announce(Onion_Client *onion_c)
{
    ...
    for (unsigned int i = 0; i < MAX_ONION_CLIENTS_ANNOUNCE; ++i) {
        ...
        if (list_nodes[i].is_stored != 0
                && path_exists(onion_c->mono_time, &onion_c->onion_paths_self, list_nodes[i].path_used)) {
