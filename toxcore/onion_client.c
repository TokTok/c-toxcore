...
static void do_friend(Onion_Client *onion_c, uint16_t friendnum)
{
    ...
    Onion_Node *nodes_list = o_friend->clients_list;

    for (unsigned i = 0; i < MAX_ONION_CLIENTS; ++i) {
        if (onion_node_timed_out(&nodes_list[i], onion_c->mono_time)) {
            continue;
        }
        ...
        if (nodes_list[i].last_pinged == 0) {
            nodes_list[i].last_pinged = tm;
            continue;
        }

        // node hasn't responded in a while so we skip it
        if (nodes_list[i].pings_since_last_response >= ONION_NODE_MAX_PINGS) {
            continue;
        }
        ...
        if (!mono_time_is_timeout(onion_c->mono_time, nodes_list[i].last_pinged, interval)) {
            continue;
        }

        if (client_send_announce_request(onion_c, friendnum + 1, &nodes_list[i].ip_port,
                                         nodes_list[i].public_key, nullptr, -1) == 0) {
            nodes_list[i].last_pinged = tm;
            o_friend->time_last_pinged = tm;
            ++nodes_list[i].pings_since_last_response;
            ++o_friend->pings;
...
static void do_announce(Onion_Client *onion_c)
{
    ...
    Onion_Node *nodes_list = onion_c->clients_announce_list;
    ...
    for (unsigned int i = 0; i < MAX_ONION_CLIENTS_ANNOUNCE; ++i) {
        ...
        if (nodes_list[i].is_stored != 0
                && path_exists(onion_c->mono_time, &onion_c->onion_paths_self, nodes_list[i].path_used)) {
            interval = ANNOUNCE_INTERVAL_ANNOUNCED;

            const uint32_t pathnum = nodes_list[i].path_used % NUMBER_ONION_PATHS;
             ...
             */
            if (mono_time_is_timeout(onion_c->mono_time, nodes_list[i].added_time, TIME_TO_STABLE)
                    && !(nodes_list[i].pings_since_last_response > 0
                         && mono_time_is_timeout(onion_c->mono_time, nodes_list[i].last_pinged, ONION_NODE_TIMEOUT))
        ...
        if (mono_time_is_timeout(onion_c->mono_time, nodes_list[i].last_pinged, interval)
            ...
            uint32_t path_to_use = nodes_list[i].path_used;

            if (nodes_list[i].pings_since_last_response == ONION_NODE_MAX_PINGS - 1
                    && mono_time_is_timeout(onion_c->mono_time, nodes_list[i].added_time, TIME_TO_STABLE)) {
                /* Last chance for a long-lived node - try a random path */
                path_to_use = -1;
            }

            if (client_send_announce_request(onion_c, 0, &nodes_list[i].ip_port, nodes_list[i].public_key,
                                             nodes_list[i].ping_id, path_to_use) == 0) {
                nodes_list[i].last_pinged = mono_time_get(onion_c->mono_time);
                ++nodes_list[i].pings_since_last_response;
