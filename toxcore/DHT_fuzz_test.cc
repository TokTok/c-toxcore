...
{
    CONSUME(const uint8_t *self_public_key, input, CRYPTO_PUBLIC_KEY_SIZE);
    CONSUME(const uint8_t *self_secret_key, input, CRYPTO_SECRET_KEY_SIZE);
...
{
    CONSUME1(const bool tcp_enabled, input);
