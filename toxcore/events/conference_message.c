...
static void tox_event_conference_message_pack(
    const Tox_Event_Conference_Message *event, cmp_ctx_t *ctx)
{
    assert(event != nullptr);
    bin_pack_array(ctx, 2);
    bin_pack_u32(ctx, TOX_EVENT_CONFERENCE_MESSAGE);
    bin_pack_array(ctx, 4);
    bin_pack_u32(ctx, event->conference_number);
    bin_pack_u32(ctx, event->peer_number);
    bin_pack_u32(ctx, event->type);
    bin_pack_bytes(ctx, event->message, event->message_length);
}
...
void tox_events_pack_conference_message(const Tox_Events *events, cmp_ctx_t *ctx)
{
    const uint32_t size = tox_events_get_conference_message_size(events);

    for (uint32_t i = 0; i < size; ++i) {
        tox_event_conference_message_pack(tox_events_get_conference_message(events, i), ctx);
    }
}
