...
void tox_events_pack_conference_message(const Tox_Events *events, Bin_Pack *bp)
{
    const uint32_t size = tox_events_get_conference_message_size(events);

    for (uint32_t i = 0; i < size; ++i) {
        tox_event_conference_message_pack(tox_events_get_conference_message(events, i), bp);
    }
}
