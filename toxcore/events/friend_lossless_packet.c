...
void tox_events_pack_friend_lossless_packet(const Tox_Events *events, Bin_Pack *bp)
{
    const uint32_t size = tox_events_get_friend_lossless_packet_size(events);

    for (uint32_t i = 0; i < size; ++i) {
        tox_event_friend_lossless_packet_pack(tox_events_get_friend_lossless_packet(events, i), bp);
    }
}
