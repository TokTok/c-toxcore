...
static void tox_event_self_connection_status_pack(
    const Tox_Event_Self_Connection_Status *event, cmp_ctx_t *ctx)
{
    assert(event != nullptr);
    bin_pack_array(ctx, 2);
    bin_pack_u32(ctx, TOX_EVENT_SELF_CONNECTION_STATUS);
...
void tox_events_pack_self_connection_status(const Tox_Events *events, cmp_ctx_t *ctx)
{
    const uint32_t size = tox_events_get_self_connection_status_size(events);

    for (uint32_t i = 0; i < size; ++i) {
        tox_event_self_connection_status_pack(tox_events_get_self_connection_status(events, i), ctx);
    }
}
