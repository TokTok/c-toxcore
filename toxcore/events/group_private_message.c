...
void tox_events_handle_group_private_message_dispatch(Tox *tox, const Tox_Event *event, void *user_data)
{
    if (tox->group_private_message_callback == nullptr) {
        return;
    }

    const Tox_Event_Group_Private_Message *ev = event->data.group_private_message;
    tox_unlock(tox);
    tox->group_private_message_callback(tox, ev->group_number, ev->peer_id, ev->message_type, ev->message, ev->message_length, ev->message_id, user_data);
    tox_lock(tox);
}
