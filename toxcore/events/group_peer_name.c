...
void tox_events_handle_group_peer_name_dispatch(Tox *tox, void *user_data, const Tox_Event *event)
{
    if (tox->group_peer_name_callback == nullptr) {
        return;
    }

    const Tox_Event_Group_Peer_Name *ev = event->data.group_peer_name;
    tox_unlock(tox);
    tox->group_peer_name_callback(tox, ev->group_number, ev->peer_id, ev->name, ev->name_length, user_data);
    tox_lock(tox);
}
