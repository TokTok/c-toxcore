...
void tox_events_handle_group_custom_packet_dispatch(Tox *tox, void *user_data, const Tox_Event *event)
{
    if (tox->group_custom_packet_callback == nullptr) {
        return;
    }

    const Tox_Event_Group_Custom_Packet *ev = event->data.group_custom_packet;
    tox_unlock(tox);
    tox->group_custom_packet_callback(tox, ev->group_number, ev->peer_id, ev->data, ev->data_length, user_data);
    tox_lock(tox);
}
