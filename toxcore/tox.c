...
static bool tox_fds(const Messenger *m, Socket **sockets, uint32_t *sockets_num)
{
    ...
    const uint32_t len = tcp_connections_length(tcp_c);
    const uint32_t fdcount = 1 + len;  // 1 UDP + len TCP sockets
    ...
    for (uint32_t i = 0; i < len; ++i) {
        ...
        assert(i < fdcount - 1);
        tmp_sockets[i + 1] = conn != nullptr ? tcp_con_sock(conn->connection) : (Socket) {
          0
        };
    }
