...
static void test_basic(void)
{
    ...
    IP ip = get_loopback();
    Onion *onion1 = new_onion(log1, mono_time1, rng, new_dht(log1, mono_time1, rng, new_networking(log1, &ip, 36567), true, false));
    Onion *onion2 = new_onion(log2, mono_time2, rng, new_dht(log2, mono_time2, rng, new_networking(log2, &ip, 36568), true, false));
    ...
    Onion *onion3 = new_onion(log3, mono_time3, rng, new_dht(log3, mono_time3, rng, new_networking(log3, &ip, 36569), true, false));
...
static Onions *new_onions(const Random *rng, uint16_t port, uint32_t *index)
{
    ...
    DHT *dht = new_dht(on->log, on->mono_time, rng, net, true, false);
    ...
    on->onion_c = new_onion_client(on->log, rng, on->mono_time, new_net_crypto(on->log, rng, on->mono_time, dht, &inf));
