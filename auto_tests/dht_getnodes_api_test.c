...
static bool all_nodes_crawled(AutoTox *autotoxes, uint32_t num_toxes, const uint8_t **public_key_list)
{
    size_t count = 0;
    ...
    for (uint32_t i = 0; i < num_toxes; ++i) {
        ...
        // make sure each peer has every other peer's public key (this is redundant but for good measure)
...
static void test_dht_getnodes(AutoTox *autotoxes)
{
    ...
    while (!all_nodes_crawled(autotoxes, NUM_TOXES, (const uint8_t **)public_key_list)) {
