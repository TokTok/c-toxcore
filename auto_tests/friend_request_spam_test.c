...
static void test_friend_request(AutoTox *autotoxes)
{
    ...
    for (uint32_t i = 2; i < TOX_COUNT; ++i) {
    ...
    printf("Tox clients connected took %lu seconds; tox1 has %d friends.\n", (unsigned long)(time(nullptr) - con_time), size);
}
...
int main(void)
{
    ...
    run_auto_test(nullptr, TOX_COUNT, test_friend_request, sizeof(State), &options);
