    ...
    Tox_Dispatch *dispatch, int interval)
{
    ...
    tox_dispatch_invoke(dispatch, events, tox1, state1);
    tox_events_free(events);

    events = tox_events_iterate(tox2, true, &err);
    ck_assert(err == TOX_ERR_EVENTS_ITERATE_OK);
    tox_dispatch_invoke(dispatch, events, tox2, state2);
    tox_events_free(events);

    events = tox_events_iterate(tox3, true, &err);
    ck_assert(err == TOX_ERR_EVENTS_ITERATE_OK);
    tox_dispatch_invoke(dispatch, events, tox3, state3);
    tox_events_free(events);
