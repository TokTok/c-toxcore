...
public:
    ...
    std::mutex &getToxLoopLock()
    {
        return tox_loop_lock;
    }
    bool inCall() const
    {
        return in_call.load();
    }
    uint32_t getCallState() const
    {
        return call_state.load();
    }
    void stopThreads()
    {
    ...
    bool didReceiveAudio() const
    {
        return audio_received.load();
    }
    bool didReceiveVideo() const
    {
        return video_received.load();
    }
