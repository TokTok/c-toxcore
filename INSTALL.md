...
- `libtoxcore.dll` is the shared library. It is fully static, with no additional
  dependencies aside from the Windows OS dlls, and can be used in MSVC, MinGW,
  Clang, etc. projects. Despite its name, it provides toxcore, toxav,
  toxencryptsave -- all of Tox.
...
- `libtoxcore.exp` and `libtoxcore.def` are the exported by `libtoxcore`
  symbols.
- `*.exe` are statically compiled executables -- `DHT_bootstrap` and [the fun
  utils](#secondary).
