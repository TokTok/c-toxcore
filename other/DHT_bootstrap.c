...
int main(int argc, char *argv[])
{
    ...
    Forwarding *forwarding = new_forwarding(logger, mono_time, dht);
...
#define NUM_PORTS 3
    ...
    TCP_Server *tcp_s = new_TCP_server(logger, ipv6enabled, NUM_PORTS, ports, dht_get_self_secret_key(dht), onion, forwarding);
