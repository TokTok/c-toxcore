...
int main(int argc, char *argv[])
{
    ...
    DHT *dht = new_dht(logger, mono_time, rng, new_networking_ex(logger, &ip, start_port, end_port, nullptr), true, true);
...
#define NUM_PORTS 3
    ...
    TCP_Server *tcp_s = new_TCP_server(logger, rng, ipv6enabled, NUM_PORTS, ports, dht_get_self_secret_key(dht), onion);
