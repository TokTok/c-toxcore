...
void RecordBootstrap()
{
    ...
    const auto iterate = [&](uint64_t clock_increment) {
    ...
    while (done1 < MESSAGE_COUNT && done2 < MESSAGE_COUNT) {
        ...
        // Less than 200 here because otherwise we'll spam onion announce packets.
        // This can't be too small because e2e_fuzz_test will always advance by
        // 200ms. If this number here is too small, e2e_fuzz_test will time out
        // the friend and won't do anything interesting.
        iterate(100);
    }
